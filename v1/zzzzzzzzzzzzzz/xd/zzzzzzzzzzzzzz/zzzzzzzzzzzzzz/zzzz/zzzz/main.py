...
class Sistema():
    def __init__(self):
        ...
        # Inicializa lista_canciones como None u otra valor por defecto
    ...
    def principal(self):
        ...
        self.menu.add_cascade(label=" Reportes ", menu=self.filemenu2, background='#2b2b2b',
                              foreground='white', activeforeground='black', activebackground='gray52')
        self.filemenu2.add_command(label="Top canciones/artistas [HTML]", command=self.top_html, background='#2b2b2b',
                                   foreground='white', activeforeground='black', activebackground='gray52')
        self.filemenu2.add_command(label="Estructura de biblioteca [GRAPHVIZ]", command=self.graficar_biblioteca, background='#2b2b2b',
                                   foreground='white', activeforeground='black', activebackground='gray52')
    ...
    def cargar_archivo(self):
        ...
        # Almacena la lista de canciones cargadas
        self.lista_canciones = self.carga.cargar_canciones()
    ...
    def cargar_archivo_defecto(self):
        ...
        # Almacena la lista de listas cargadas
        self.lista_listas = self.carga_lista.cargar_listas()
    ...
    def buscar_lista(self):
        ...
        while listas_en:
            ...
            if valor_entrada == valor_comparado:
                nuevo_dato = listas_en.dato.reproduccion
                nuevo_dato = int(nuevo_dato)+1
                listas_en.dato.reproduccion = str(nuevo_dato)
    ...
    def reproducir_cancion_siguiente(self):
        if self.boton_reproduccion_normal['state'] == 'normal' and self.boton_reproduccion_aleatoria['state'] == 'disabled':
            ...
            if cancion_aleatoria:
                ...
                self.mostrar_imagen(cancion_aleatoria.imagen)
            else:
            ...
            if siguiente_cancion:
                ...
                self.mostrar_imagen(siguiente_cancion.imagen)
            else:
                # Manejo si no hay siguiente canción disponible
                self.entry_cancion.delete(0, END)
                self.entry_cancion.insert(0, "No hay más canciones")
    ...
    def reproducir_cancion_anterior(self):
        ...
        if cancion_anterior:
            ...
            self.mostrar_imagen(cancion_anterior.imagen)
        else:
    ...
    def anadir_lista_cancion(self):
        ...
        while listas_en:
            if listas_en.dato.lista == valor_entrada_lista:
                lista_existente = listas_en.dato
                break
            listas_en = listas_en.siguiente
        ...
        if lista_existente:
            ...
            while canciones_en:
                if canciones_en.dato.cancion == valor_entrada_cancion:
                    cancion_existente = canciones_en.dato
                    break
                canciones_en = canciones_en.siguiente

            if cancion_existente:
                valor_salida = Playlist(
                    valor_entrada_lista, lista_existente.reproduccion, cancion_existente.cancion, cancion_existente.artista, cancion_existente.album, cancion_existente.imagen, cancion_existente.ruta)

                self.carga_lista.lista_listas.agregar_al_final(valor_salida)
        else:
            nueva_reproduccion = "0"
            cancion_existente = None
            while canciones_en:
                if canciones_en.dato.cancion == valor_entrada_cancion:
                    cancion_existente = canciones_en.dato
            ...
            if cancion_existente:
                valor_salida = Playlist(
                    valor_entrada_lista, nueva_reproduccion, cancion_existente.cancion, cancion_existente.artista, cancion_existente.album, cancion_existente.imagen, cancion_existente.ruta)
                self.carga_lista.lista_listas.agregar_al_final(valor_salida)

    def anadir_lista_artista(self):
        valor_entrada_lista = self.entry_lista2.get()
        valor_entrada_artista = self.entry_artista.get()
        ...
        if lista_existente:
            artista_existente = None
            while canciones_en:
                if canciones_en.dato.artista == valor_entrada_artista:
                    artista_existente = canciones_en.dato
                    break
                canciones_en = canciones_en.siguiente

            if artista_existente:
                valor_salida = Playlist(
                    valor_entrada_lista, lista_existente.reproduccion, artista_existente.cancion, artista_existente.artista, artista_existente.album, artista_existente.imagen, artista_existente.ruta)
            ...
            if artista_existente:
                valor_salida = Playlist(
                    valor_entrada_lista, nueva_reproduccion, artista_existente.cancion, artista_existente.artista, artista_existente.album, artista_existente.imagen, artista_existente.ruta)
                self.carga_lista.lista_listas.agregar_al_final(valor_salida)
    ...
    def graficar_biblioteca(self):
        self.carga_lista.graficar()

    def top_html(self):
        self.carga_lista.generar_html()
